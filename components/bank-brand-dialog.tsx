...
import { createBankBrandContribution } from "@/actions/contribution-actions";
...
export function BankBrandDialog() {
  ...
  const [existingBankBrand, setExistingBankBrand] = useState<
    WeroData["bankBrands"][number] | null
  >(null);
  ...
  useEffect(() => {
    onOpenContributionDialog((options) => {
      if (options.type === "bank-brand") {
        setOpen(true);
        setAction(options.action);
        if (options.action === "edit" || options.action === "delete") {
          setExistingBankBrand(options.entity);
        } else {
          setExistingBankBrand(null);
          resetForm();
        }
      }
    });
    ...
    if (open) {
      if (existingBankBrand) {
        ...
        const bankData = existingBankBrand.banks.map((bank) => ({
          ...
          appIds: bank.bankingAppsToBanks.map((bab) => bab.appId),
        }));
        setBanks(bankData);
        setShowBanksSection(bankData.length > 0);
        // Populate apps
        const appData = existingBankBrand.banks
          .map((bank) => bank.bankingAppsToBanks)
          .flat()
          .map((bab) => bab.bankingApp)
          // unique
          .filter(
            (app, index, self) =>
  ...
  const handleSubmit = async () => {
    ...
    if (action === "add") {
      const { success, message } = await createBankBrandContribution({
        action,
        data: {
          brand: {
            name,
